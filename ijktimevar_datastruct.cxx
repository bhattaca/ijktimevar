...
void POLY_ISOTABLE::ComputeAmbiguityInformation()
{
  if (cube.IsTableAllocated())
    { ambig_cube.SetAmbiguityTable(cube.Polyhedron()); };

  if (pyramid.IsTableAllocated())
    { ambig_pyramid.SetAmbiguityTable(pyramid.Polyhedron()); };
}
...
void MC_DATA::SetMinmaxRegions(const int region_length)
{
  ...
  minmax = new IJKTIMEVAR::MINMAX_REGIONS();
...
(const std::vector<GRID_BOX> & high_resolution_regions)
{
  ...
  for (unsigned int i = 0; i < high_resolution_regions.size(); i++) {
...
IJKTIMEVAR::MCUBE_TIME::MCUBE_TIME()
// constructor
{
  Clear();
}

void IJKTIMEVAR::MCUBE_TIME::Clear()
{
  preprocessing = 0.0;
...
void IJKTIMEVAR::MCUBE_TIME::Add(const MCUBE_TIME & mcube_time)
{
...
IJKTIMEVAR::GRID_INFO::GRID_INFO()
{
  Clear();
}

void IJKTIMEVAR::GRID_INFO::Clear()
{
  num_cubes = 0;

}

void IJKTIMEVAR::SCALAR_INFO::Init(const int dimension)
{
...
void IJKTIMEVAR::SCALAR_INFO::FreeAll()
{
...
void IJKTIMEVAR::SCALAR_INFO::Clear()
{
...
void IJKTIMEVAR::SCALAR_INFO::SetDimension(const int dimension)
{
  FreeAll();
...
void IJKTIMEVAR::SCALAR_INFO::Copy(const SCALAR_INFO & info)
{
  Init(info.Dimension());
...
/// Copy assignment.
const SCALAR_INFO &  IJKTIMEVAR::SCALAR_INFO::operator =
...
IJKTIMEVAR::SCALAR_INFO::~SCALAR_INFO()
{
  delete [] num_cubes_with_saddle;
  num_cubes_with_saddle = NULL;
  dimension = 0;
...
IJKTIMEVAR::MCUBE_INFO::MCUBE_INFO()
{
  Clear();
}

IJKTIMEVAR::MCUBE_INFO::MCUBE_INFO(const int dimension):scalar(dimension)
{
  Clear();
}

void IJKTIMEVAR::MCUBE_INFO::Clear()
{
  grid.Clear();
  scalar.Clear();
  time.Clear();
...
void IJKTIMEVAR::NEP_INFO::Clear()
{
...
void IJKTIMEVAR::MERGE_DATA::Init
...
bool IJKTIMEVAR::MERGE_DATA::Check(ERROR & error) const
{
  if (MaxNumInt() < 
...
bool IJKTIMEVAR::MERGE_DATA::Check
...
void IJKTIMEVAR::SNAP_MINMAX::ComputeMinMax
(const int dimension, const AXIS_SIZE_TYPE * axis_size,
...
void IJKTIMEVAR::SNAP_OCTREE::SetMinMax(const SCALAR_TYPE * scalar)
{
