...
(const int vertex_dimension, 
 ...
 const int * simplex_vert, const int num_simplices, 
 const int slice_axis, const COORD_TYPE slice_coord,
 vector<int> & elist, vector<int> & slist);
  ...
  // num_slice_simplices = number of slice simplices (returned)
{
  ...
  if (!read_isotable(simplex_dimension, isotable_directory))
    return;
  cout <<"start locate intersect"<<endl;  
  locate_intersected_edges(vertex_dimension, vertex_coord, num_vert, 
			   simplex_dimension, simplex_vert, num_simplices, 
			   slice_axis, slice_coord, intersected_e);
  std::cout <<"test "<<std::endl;  
  construct_sliced_simplices
    (vertex_dimension, vertex_coord, num_vert, 
     simplex_dimension, simplex_vert, num_simplices, 
     slice_axis, slice_coord, intersected_e, slice_s);
  ...
  // located intersected edges and store endpoints in elist
{
	//DEBUG
	using namespace std;
	cerr <<" num simplices " << num_simplices << endl;

	for (int is = 0; is < num_simplices; is++) {
		//DEBUG
		if (is%10000==0)
			cerr <<"is "<< is << " , elist size "<< elist.size() << endl;
...
void insert(vector<int> & elist, const int v0, const int v1)
  ...
  // ie = index of edge found; undefined if edge not found
{
  ...
  for (int i = 0; i < elist.size()/2; i++) {
    if (elist[2*i] == u0 && elist[2*i+1] == u1) {
      ie = i;
      return(true);
    };
  };
...
//   {x_{slice_axis} = slice_coord}
{
  ...
  for (int is = 0; is < num_simplices; is++) {
	...
	int v1 = vlist[jv1];
	int ie;
	bool found_flag = find(elist, v0, v1, ie);
